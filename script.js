...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const templeBgV12 = document.querySelector(".temple-bg-image");
  let templeRafV12 = false;
  function moveTempleV12(){
    if(!templeBgV12) return;
    const y = Math.min(window.scrollY * 0.05, 32);
    const s = 1.03 + Math.min(window.scrollY / 45000, 0.02);
    templeBgV12.style.transform = `translate(-50%, calc(-50% + ${y}px)) scale(${s})`;
    templeRafV12 = false;
  }
  window.addEventListener("scroll",()=>{
    if(!templeRafV12){
      requestAnimationFrame(moveTempleV12);
      templeRafV12 = true;
    }
  },{passive:true});
  moveTempleV12();
